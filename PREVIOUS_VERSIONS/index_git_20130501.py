...
def normalize_words(strings, wnl_obj):
    ...
    for the_string in strings:
        words_to_divide = []
        words_not_to_divide = []
        hash_of_whole_string = the_string.__hash__()
...
def prepare_tuple_storage(tuple_storage, all_strings, path_type, the_path, the_string):
    tuple_storage[the_string.__hash__()] = (path_type, the_path, the_string)
...
def main():
    ...
    print len(words_and_their_string_hashes), len(index_entries)
    for word in index_entries:
        pprint.pprint(word)
        for item in index_entries[word]:
            pprint.pprint(tuple_storage[item])
    pprint.pprint(json.dumps(index_entries))
    pprint.pprint(json.dumps(tuple_storage))
